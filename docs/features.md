...
- **Service ML isolé** (`ml-service/`) avec **fallback in-process** (jamais de panne dure).
...
- **Pages** : Briefing du matin, Recommandations, Alertes, Démarque, Production,
  Prévisions, Stocks, Catalogue & prix, Boucherie, Promos, Finance, Factures,
  Fournisseurs, Fin de journée, Qualité, Chaîne du froid, Data Ops, Connecteurs,
  Intégrations, Assistant (chat).
...
- **Catalogue & prix** : produits, familles, prix, historique.
...
- **WhatsApp** (mock | Business API), **Telegram** (mock | bot), **Slack** (mock | bot +
  Events API), **chat web**.
- **État des connecteurs sans secret** (`/config/connectors`), page Connecteurs.
...
- **RBAC** par rôle (owner/manager/staff/viewer) + scopes.
...
- **Tests E2E** Playwright, **CI** multi-jobs (backend, pg, frontend, website, ml-service,
  e2e, docker), **observabilité** (health étendu, métriques pipelines).
